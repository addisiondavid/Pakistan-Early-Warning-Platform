...
Array.prototype.pip = function(x, y) {
	...
	for (i = 0, j = this.length - 1; i < this.length; j = i++) {
		if (((this[i][1] > y) != (this[j][1] > y)) && (x < (this[j][0] - this[i][0]) * (y - this[i][1]) / (this[j][
				1] - this[i][1]) + this[i][0])) {
			c = !c
		}
	}
...
var kriging = function() {
	...
	kriging.train = function(t, x, y, model, sigma2, alpha) {
		...
		for (i = 0; i < n; i++) {
			switch (model) {
				...
				case "exponential":
					...
					break;
				case "spherical":
					X[i * 2 + 1] = 1.5 * (lag[i] / variogram.range) - 0.5 * Math.pow(lag[i] / variogram.range,
						3);
					break
			};
			Y[i] = semi[i]
		...
		for (i = 0; i < n; i++) {
			for (j = 0; j < i; j++) {
				K[i * n + j] = variogram.model(Math.pow(Math.pow(x[i] - x[j], 2) + Math.pow(y[i] - y[j], 2),
					0.5), variogram.nugget, variogram.range, variogram.sill, variogram.A);
				K[j * n + i] = K[i * n + j]
			}
	...
	kriging.predict = function(x, y, variogram) {
		...
		for (i = 0; i < variogram.n; i++) k[i] = variogram.model(Math.pow(Math.pow(x - variogram.x[i], 2) + Math
				.pow(y - variogram.y[i], 2), 0.5), variogram.nugget, variogram.range, variogram.sill,
			variogram.A);
		...
		for (i = 0; i < n; i++) {
			...
			for (j = a[0]; j <= a[1]; j++)
				for (k = b[0]; k <= b[1]; k++) {
					...
					if (polygons[i].pip(xtarget, ytarget)) A[j][k] = kriging.predict(xtarget, ytarget,
						variogram)
				}
		}
		A.xlim = xlim;
	...
	kriging.plot = function(canvas, grid, xlim, ylim, colors) {
		console.info(canvas, grid, xlim, ylim, colors)
